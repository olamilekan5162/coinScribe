...
const Profile: React.FC = () => {
  ...
  const { isFollowing, toggleFollow, loading: followLoading } = useFollow(currentUser?.id);
  
  const [profileUser, setProfileUser] = useState<ProfileUser | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'posts' | 'drafts' | 'liked'>('posts');
  ...
  const isOwnProfile = currentUser?.wallet_address.toLowerCase() === address?.toLowerCase();
            ...
            <div className={styles.profileMeta}>
              ...
              <div className={styles.stats}>
                <div className={styles.stat}>
                  <span className={styles.statValue}>{formatNumber(profileUser.followers_count)}</span>
                  <span className={styles.statLabel}>Followers</span>
                </div>
                <div className={styles.stat}>
                  <span className={styles.statValue}>{formatNumber(profileUser.following_count)}</span>
                  <span className={styles.statLabel}>Following</span>
                </div>
                <div className={styles.stat}>
                  <span className={styles.statValue}>{profileUser.posts_count}</span>
                  <span className={styles.statLabel}>Posts</span>
                </div>
                <div className={styles.stat}>
                  <span className={styles.statValue}>${formatNumber(profileUser.total_earnings)}</span>
                  ...
                  {posts.map((post) => (
                    <PostCard 
                      ...
                      post={{
                        ...
                        coinData: {
                          price: post.coin_price,
                          change: post.price_change,
                          holders: post.holders_count
                        }
                      }}
                    />
                  ))}
